...
@HiltAndroidTest
class MediaControlsSettingsScreenTest {
    ...
    fun `Given loading state when displayed then no entity configuration is offered`() {
        composeTestRule.apply {
            testScreen(MediaControlsSettingsUiState(isLoading = true)) {
                onNodeWithText(stringResource(R.string.media_controls_description)).assertIsDisplayed()
                onNodeWithText(stringResource(R.string.media_controls_select_entity)).assertDoesNotExist()
    ...
    fun `Given a single server when displayed then the server selector is hidden`() {
        composeTestRule.apply {
            testScreen(singleServerState) {
                onNodeWithText(stringResource(R.string.server_select)).assertDoesNotExist()
            }
        }
    }
    ...
    fun `Given several servers when displayed then the server selector is shown`() {
        composeTestRule.apply {
            testScreen(multipleServersState) {
                onNodeWithText(stringResource(R.string.server_select), substring = true).assertIsDisplayed()
            }
        }
    }
    ...
    fun `Given a configured entity when remove is clicked then onRemoveEntity is invoked with it`() {
        composeTestRule.apply {
            testScreen(singleServerState) {
                onAllNodesWithContentDescription(stringResource(R.string.media_controls_remove_entity))[0]
                    .performClick()

                Assertions.assertEquals(livingRoomConfig, removedEntity?.config)
            }
        }
    }
    ...
    fun `Given several servers when another one is selected then onServerSelected is invoked with its id`() {
        composeTestRule.apply {
            testScreen(multipleServersState) {
                onNodeWithText(stringResource(R.string.server_select), substring = true).performClick()
                waitForIdle()
                ...
                Assertions.assertEquals(OTHER_SERVER_ID, selectedServerId)
            }
        }
    }
    ...
    fun `Given a configured entity when the picker is expanded then it is not offered again`() {
        composeTestRule.apply {
            testScreen(singleServerState) {
                onNodeWithText(stringResource(R.string.media_controls_select_entity)).performClick()
                waitForIdle()
    ...
    fun `Given the picker when an entity is selected then onEntitySelected is invoked with its id`() {
        composeTestRule.apply {
            testScreen(singleServerState) {
                ...
                Assertions.assertEquals(BEDROOM_ENTITY_ID, selectedEntityId)
            }
        }
    }
    ...
    private class TestHelper {
        ...
        var removedEntity: MediaControlsSelectedEntity? = null
    }
    ...
    private companion object {
        ...
        val livingRoomConfig =
            MediaControlsEntityConfig(serverId = SERVER_ID, entityId = "media_player.living_room")
        val kitchenConfig =
            MediaControlsEntityConfig(serverId = SERVER_ID, entityId = "media_player.kitchen")
        val officeConfig =
            MediaControlsEntityConfig(serverId = OTHER_SERVER_ID, entityId = OFFICE_ENTITY_ID)
        ...
        val singleServerState = MediaControlsSettingsUiState(
            isLoading = false,
            selectedServerId = SERVER_ID,
            serversDropdownItems = listOf(HADropdownItem(key = SERVER_ID, label = HOME_SERVER_NAME)),
            mediaControlsEntityConfigs = listOf(livingRoomConfig, kitchenConfig),
        ...
        val multipleServersState = singleServerState.copy(
            ...
            mediaControlsEntityConfigs = listOf(livingRoomConfig, kitchenConfig, officeConfig),
        )
    }
}
