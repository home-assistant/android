...
import io.homeassistant.companion.android.common.data.mediacontrols.MediaControlsRepository
...
class MediaControlsSettingsViewModelTest {
    ...
    private val mediaControlsRepository: MediaControlsRepository = mockk(relaxed = true)
    private val entitiesForDisplayManager: EntitiesForDisplayManager = mockk(relaxed = true)

    private val configuredEntitiesFlow = MutableStateFlow<List<MediaControlsEntityConfig>>(emptyList())
    private val serversFlow = MutableStateFlow(listOf(fakeServer(DEFAULT_SERVER_ID)))

    private lateinit var viewModel: MediaControlsSettingsViewModel
    ...
    fun setUp() {
        ...
        coEvery { mediaControlsRepository.observeEntities() } returns configuredEntitiesFlow
        coEvery { mediaControlsRepository.addEntity(any()) } coAnswers {
            // The (server_id, entity_id) primary key makes a repeated insert a replace
            val config = firstArg<MediaControlsEntityConfig>()
            configuredEntitiesFlow.value = configuredEntitiesFlow.value - config + config
        }
        coEvery { mediaControlsRepository.removeEntity(any()) } coAnswers {
            configuredEntitiesFlow.value = configuredEntitiesFlow.value - firstArg<MediaControlsEntityConfig>()
        }
    ...
    private fun createViewModel(): MediaControlsSettingsViewModel {
        return MediaControlsSettingsViewModel(
            serverManager = serverManager,
            mediaControlsRepository = mediaControlsRepository,
    ...
    inner class InitializationTest {
        ...
        fun `Given no configured entities when viewModel created then selectedEntities is empty`() = runTest(testDispatcher) {
            ...
            assertEquals(emptyList<MediaControlsSelectedEntity>(), viewModel.uiState.value.selectedEntities)
        }

        @Test
        fun `Given configured entities when viewModel created then selectedEntities reflects repo`() = runTest(testDispatcher) {
            configuredEntitiesFlow.value = listOf(MediaControlsEntityConfig(serverId = 1, entityId = "media_player.tv"))
    ...
    inner class AvailableEntitiesTest {
        ...
        fun `Given a registered server when its entity is configured then the server name is resolved`() = runTest(testDispatcher) {
            ...
            configuredEntitiesFlow.value = listOf(
                MediaControlsEntityConfig(serverId = DEFAULT_SERVER_ID, entityId = "media_player.tv"),
            )
        ...
        fun `Given an unregistered server when its entity is configured then the server id is the name`() = runTest(testDispatcher) {
            configuredEntitiesFlow.value = listOf(
                MediaControlsEntityConfig(serverId = OTHER_SERVER_ID, entityId = "media_player.tv"),
            )
        ...
        fun `Given resolved entities when a configured entity belongs to them then its display is resolved`() = runTest(testDispatcher) {
            ...
            configuredEntitiesFlow.value = listOf(
                MediaControlsEntityConfig(serverId = DEFAULT_SERVER_ID, entityId = "media_player.tv"),
                MediaControlsEntityConfig(serverId = OTHER_SERVER_ID, entityId = "media_player.radio"),
            )
    ...
    inner class AddEntityTest {
        ...
        fun `Given viewModel when addEntity called then repository updated and start event emitted`() = runTest(testDispatcher) {
            ...
            viewModel.serviceEvents.test {
                ...
                coVerify {
                    mediaControlsRepository.addEntity(
    ...
    inner class RemoveEntityTest {
        ...
        fun `Given one entity when removeEntity called then repository cleared and no event emitted`() = runTest(testDispatcher) {
            ...
            viewModel.serviceEvents.test {
                ...
                coVerify { mediaControlsRepository.removeEntity(match { it.entityId == "media_player.tv" }) }
                expectNoEvents()
                cancelAndIgnoreRemainingEvents()
            }
        }
        ...
        fun `Given two entities when removeEntity called then repository updated and start event emitted`() = runTest(testDispatcher) {
            ...
            viewModel.serviceEvents.test {
                ...
                coVerify {
                    mediaControlsRepository.removeEntity(match { it.entityId == "media_player.tv" })
                }
                assertEquals(MediaControlServiceEvent.Start, awaitItem())
                cancelAndIgnoreRemainingEvents()
            }
