...
class HaMediaSessionService @VisibleForTesting constructor(private val serviceScope: CoroutineScope) :
    ...
    @Inject
    lateinit var mediaControlsRepository: MediaControlsRepository

    @Inject
    lateinit var haMediaSessionFactory: HaMediaSession.Factory

    /** Keyed by [MediaControlsEntityConfig.id]. Each entry pairs the session with the coroutine running [HaMediaSession.observe]. */
    ...
    internal fun startObservingEntities() {
        mediaControlsRepository.observeEntities()
            .onEach { entities -> reconcileSessions(entities) }
            .launchIn(serviceScope)
    }

    private suspend fun reconcileSessions(configuredEntities: List<MediaControlsEntityConfig>) {
