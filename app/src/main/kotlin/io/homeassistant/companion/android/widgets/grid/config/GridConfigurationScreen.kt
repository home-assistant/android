...
@Composable
fun GridWidgetConfigurationScreen(
    viewModel: GridConfigurationViewModel,
    onAddWidget: (GridConfiguration) -> Unit,
...
private fun GridWidgetConfigurationContent(
    ...
    Scaffold(
        ...
        Column(modifier.padding(padding)) {
            PreferenceSection {
                var name by remember { mutableStateOf(config.label.orEmpty()) }
                TextField(
                    modifier = Modifier
                        .fillMaxWidth()
                        .onFocusChanged { if (!it.isFocused) onNameChange(name) },
                    value = name,
