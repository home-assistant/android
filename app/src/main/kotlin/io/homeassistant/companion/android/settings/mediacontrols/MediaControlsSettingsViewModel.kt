...
/** One-shot events emitted by [MediaControlsSettingsViewModel] for the UI layer to act on. */
 ...
 * null until the entities of [MediaControlsEntityConfig.serverId] are resolved.
 *
 * [serverName] falls back to [MediaControlsEntityConfig.serverId] when the server is not registered
 * anymore, so the row still identifies where the orphaned entity came from.
 */
data class MediaControlsSelectedEntity(
    val config: MediaControlsEntityConfig,
    val entityForDisplay: EntityDisplayWithContext?,
    val serverName: String,
)

@Stable
data class MediaControlsSettingsUiState(
    val selectedServerId: Int = ServerManager.SERVER_ID_ACTIVE,
    val serversDropdownItems: List<HADropdownItem<Int>> = emptyList(),
    val mediaControlsEntityConfigs: List<MediaControlsEntityConfig> = emptyList(),
    ...
    val availableEntities: EntityDisplayState<EntityDisplayWithContext> =
        when (val displayState = entityDisplayStatePerServer[selectedServerId]) {
            ...
            is EntityDisplayState.Loaded -> {
                val configured = mediaControlsEntityConfigs
    ...
    val selectedEntities: List<MediaControlsSelectedEntity> = mediaControlsEntityConfigs.map { config ->
        MediaControlsSelectedEntity(
            config = config,
...
@HiltViewModel
class MediaControlsSettingsViewModel @Inject constructor(
    private val serverManager: ServerManager,
    private val mediaControlsRepository: MediaControlsRepository,
    private val entitiesForDisplayManager: EntitiesForDisplayManager,
) : ViewModel() {

    private val _uiState = MutableStateFlow(MediaControlsSettingsUiState())
    val uiState: StateFlow<MediaControlsSettingsUiState> = _uiState.asStateFlow()
    ...
    init {
        ...
        viewModelScope.launch {
            mediaControlsRepository.observeEntities().collect { dbConfigs ->
                _uiState.update { state ->
                    state.copy(
                        mediaControlsEntityConfigs = dbConfigs,
                        isLoading = false,
                    )
                }
    ...
    fun addEntity(entityId: String) {
        viewModelScope.launch {
            ...
            val config = MediaControlsEntityConfig(serverId = serverId, entityId = entityId)
            mediaControlsRepository.addEntity(config)
        }
    }
     ...
     */
    fun removeEntity(selectedEntity: MediaControlsSelectedEntity) {
        viewModelScope.launch {
            mediaControlsRepository.removeEntity(selectedEntity.config)
        }
    }
