...
import io.shpro.companion.android.onboarding.integration.MobileAppIntegrationActivity
import io.shpro.companion.android.onboarding.phoneinstall.PhoneInstallActivity
import io.shpro.companion.android.util.LoadingView
    ...
    private fun requestInstances() {
        ...
        capabilityInfo.nodes.forEach { node ->
            Wearable.getMessageClient(this).sendMessage(
                ...
                addOnSuccessListener { Log.d(TAG, "requestInstances: request SH Pro instances from $node.id: ${node.displayName}") }
                addOnFailureListener { Log.w(TAG, "requestInstances: failed to request SH Pro instances from $node.id: ${node.displayName}") }
            }
        }
    }
