 ...
 * - [migrateFromVersion24ToLatest] runs the whole chain from the earliest exported schema (v24)
 *   and lets Room validate the final schema. Schemas for v1-23 were never exported, so the helper
 *   cannot seed those versions.
 * - The `migrationX_toY_*` tests exercise the manually written migrations that were rewritten to
 *   the SQLite driver APIs, by building the old table shape directly on a driver connection,
 *   seeding representative rows, running the migration and asserting the resulting data. This both
 *   covers the pre-v24 migrations the helper cannot reach and guards against regressions in the
 *   driver rewrite.
 */
...
class AppDatabaseMigrationTest {
    ...
    @Test
    fun migrateFromVersion24ToLatest() = runBlocking {
        helper.createDatabase(24).close()
        helper.runMigrationsAndValidate(LATEST_VERSION, migrationPath(context).toList()).close()
    }

    @Test
    fun migration5to6_restructuresStaticWidgetAndKeepsData() = runBlocking {
    ...
    @Test
    fun migration16to17_renamesSettingsAndPreservesFirstRowSkip() = runBlocking {
        val connection = openDirectDb()
        try {
            connection.execSQL(
