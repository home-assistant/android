package io.shpro.companion.android.common.sensors
...
import io.shpro.companion.android.common.util.CHANNEL_SENSOR_SYNC
import io.shpro.companion.android.database.AppDatabase
import io.shpro.companion.android.database.sensor.SensorDao
import io.shpro.companion.android.database.sensor.SensorWithAttributes
import io.shpro.companion.android.database.sensor.toSensorWithAttributes
import io.shpro.companion.android.database.sensor.toSensorsWithAttributes
...
abstract class SensorReceiverBase : BroadcastReceiver() {
    companion object {
        const val ACTION_UPDATE_SENSOR = "io.shpro.companion.android.UPDATE_SENSOR"
        const val ACTION_UPDATE_SENSORS = "io.shpro.companion.android.UPDATE_SENSORS"
        const val ACTION_STOP_BEACON_SCANNING = "io.shpro.companion.android.STOP_BEACON_SCANNING"
                    ...
                    if (setting.value != "" && intent.action == setting.value) {
                        ...
                        sensorDao.get(LastUpdateManager.lastUpdate.id).forEach { sensor ->
                            ...
                            try {
                                ...
                                Log.d(tag, "Event successfully sent to SH Pro")
                            } catch (e: Exception) {
                                Log.e(tag, "Unable to send event data to SH Pro", e)
                            }
                        }
                    }
                }
